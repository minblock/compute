...
const string strMessageMagic = "ComputeCore Signed Message:\n";
...
{
    // ComputeCore: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // ComputeCore: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // ComputeCore: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ComputeCore: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ComputeCore: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ComputeCore: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0x4c, 0x65, 0x65, 0x74 }; // LEET
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ComputeCoreMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ComputeCoreMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ComputeCoreMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running ComputeCoreMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ComputeCoreMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ComputeCoreMiner, pwallet));
}
