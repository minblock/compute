...
{
    // Convert ovocoin:// to ovocoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ovocoin://"))
    {
        uri.replace(0, 12, "ovocoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OVOCoin.lnk";
}
...
{
    return GetAutostartDir() / "ovocoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OVOCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ovocoin-qt [" + tr("command-line options") + "]                     " + "\n";
