...
// which are part of the Compute repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="compute-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /compute-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "computepay-compute/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from computepay-compute/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'computepay-compute/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from computepay-compute/develop"
            }
          }

          if (hasCache) {
            sh "cd /compute-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /compute-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /compute-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /compute-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /compute-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /compute-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /compute-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
