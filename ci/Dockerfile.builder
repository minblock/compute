...
# compute_hash
RUN git clone https://github.com/computepay/compute_hash
RUN cd compute_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} compute
RUN useradd -u ${USER_ID} -g compute -s /bin/bash -m -d /compute compute
...
RUN mkdir /compute-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /compute-src && \
