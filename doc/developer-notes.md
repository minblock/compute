...
Compute Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as Compute Core, where everyone uses
...
pay attention to for reviewers of Compute Core code.

General Compute Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Compute Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Compute Core so that they can be integrated
