...
### Send 1000 COMPUTE to New Addresses

Send exactly 1000 COMPUTE to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 COMPUTE.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 COMPUTE on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the compute github repo](https://github.com/minblock/compute/blob/master/doc/masternode_conf.md)
...
## Update compute.conf on server

If you generated a new masternode private key, you will need to update the remote `compute.conf` files.

Shut down the daemon and then edit the file.

```nano .computecore/compute.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `compute.conf` file.
...
```
compute-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<1000 COMPUTE address>",
...
Search your Masternodes on https://computeninja.pl/masternodes.html
