...
Compute now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
Format: ```mngovernance prepare proposal-name url payment-count block-start compute-address monthly-payment-compute```
...
Format: ```mngovernance submit proposal-name url payment-count block-start compute-address monthly-payment-compute fee-tx```
